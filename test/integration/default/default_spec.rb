...
describe json('/etc/sensu/secrets_providers/vault.json') do
  ...
  its(%w(spec client tls ca_cert)) { should eq '/path/to/your/ca.pem' }
  its(%w(spec client tls client_cert)) { should eq '/path/to/backend/pem/for/vault.pem' }
  its(%w(spec client tls client_key)) { should eq '/path/to/backend/key/for/vault.pem' }
  its(%w(spec client tls cname)) { should eq 'sensu-backend.example.com' }
end
